...
fn generate_code(mut module: ast::ModModule) -> Result<String, anyhow::Error> {
    ...
    let mut generator = Generator::new(&indent, LineEnding::Lf);
    ...
    // call `generate` (which is NOT public) and `stmt` and `expr` requires an single argument with a stmt or expr.
